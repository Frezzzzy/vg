    ...
    internal class Program
    {
        ...
        public static string[] Stein =
       {
        ...
        public static int Minimax(int depth, int alpha, int beta, bool maximizingPlayer)
        {
            ...
            if (depth == 0)
            {
                return score; // if we have reached the maximum depth, return the score
            }
            if (maximizingPlayer)
            {
                evaluate(1);
                ...
                for (int column = 0; column < 7; column++)
                {
                    for (int row = 5; row >= 0; row--)
                    {
                        ...
                        int newScore = Minimax(depth - 1, alpha, beta, false); // recursively call Minimax with the next depth
                        mx[row, column] = 0; // undo the move
                        bestScore = Math.Max(bestScore, newScore); // update the best score
            ...
            else
            {
                ...
                for (int column = 0; column < 7; column++)
                {
                    for (int row = 5; row >= 0; row--)
                    {
                        ...
                        mx[row, column] = -1; // make the move
                        int newScore = Minimax(depth - 1, alpha, beta, true); // recursively call Minimax with the next depth
                        mx[row, column] = 0; // undo the move
                        bestScore = Math.Min(bestScore, newScore); // update the best score
        ...
        public static void Comp_einwurf(int skg) //[1,2] row 1 und column 2 Oliver if-Simulator
        {
            ...
            if (skg == 0)//leicht 
            {
                Minimax(40, -100, 100, true); //Maximizing Player ist der Bot minimizing der Bot gegen sich selbst?
